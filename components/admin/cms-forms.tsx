...
export function ReusableEntryForm({ entry }: { entry?: { id: string; entry_type: string; key: string; content: unknown } }) {
  ...
  const [title, setTitle] = useState(String(initialContent.question ?? initialContent.title ?? ""));
  const [body, setBody] = useState(String(initialContent.answer ?? initialContent.body ?? ""));
  const content = useMemo(() => entryType === "faq" ? { question: title, answer: body } : { title, body }, [body, entryType, title]);
  return <form action={action} className="admin-form admin-form-grid">{entry ? <input type="hidden" name="entryId" value={entry.id} /> : null}<div className="admin-field"><label>Entry type</label><select aria-label="Entry type" name="entryType" value={entryType} onChange={(event) => setEntryType(event.target.value)}><option value="faq">FAQ</option><option value="resource">Resource</option><option value="credential">Credential</option><option value="pricing_note">Pricing note</option><option value="legal_notice">Legal notice</option><option value="testimonial">Testimonial</option></select></div><Field name="key" label="Internal key" defaultValue={entry?.key} required help="Lowercase words separated with hyphens." /><div className="admin-field"><label>{entryType === "faq" ? "Question" : "Title"}</label><input aria-label={entryType === "faq" ? "Question" : "Title"} value={title} onChange={(event) => setTitle(event.target.value)} required /></div><div className="admin-field admin-field-wide"><label>{entryType === "faq" ? "Answer" : "Body"}</label><textarea aria-label={entryType === "faq" ? "Answer" : "Body"} value={body} onChange={(event) => setBody(event.target.value)} required /></div><input type="hidden" name="status" value="draft" /><input type="hidden" name="content" value={JSON.stringify(content)} /><div className="admin-form-footer"><ActionFeedback state={state} /><SubmitButton>Save reusable draft</SubmitButton></div></form>;
}
