...
export default async function BookingDetailPage({ params }: { params: Promise<{ bookingId: string }> }) {
  ...
  const [{ data: booking }, { data: events }, { data: sync }] = await Promise.all([
    supabase.from("bookings").select("*,services(name,price_cents)").eq("id", bookingId).maybeSingle(),
  ...
  if (!booking) notFound();
  const service = booking.services as {name:string;price_cents:number};
