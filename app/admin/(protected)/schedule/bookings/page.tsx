...
export default async function BookingsPage() {
  ...
  const { data } = await supabase.from("bookings").select("id,public_reference,starts_at,state,hold_expires_at,services(name)").order("starts_at", { ascending: false }).limit(200);
